! João Faria (Feb 2013)
! included module gnufor2 as "lib_plot" in fortran libraries
module lib_plot
...
!***********************************************************************************
	...
	character(len=*),optional	:: style, color1, color2, color3, color4, terminal, filename, polar,&
						           & persist, input, logscale
    real(kind=8), intent(in), optional    :: xrange(2), yrange(2)
	...
	real(kind=8),dimension(2) :: my_xrange, my_yrange
!***********************************************************************************
	if (present(input)) then
		data_file_name='data_file_'//input//'.txt'
		command_file_name='command_file_'//input//'.txt'		
...
!***********************************************************************************
	...
	if (present(xrange)) then
	    my_xrange(1) = xrange(1)
	    my_xrange(2) = xrange(2)	
	    write ( file_unit, '(a, f6.1, a, f6.1, a)' ) &
	        'set xrange [', my_xrange(1), ':', my_xrange(2), ']'
	endif
	if (present(yrange)) then
	    my_yrange(1) = yrange(1)
	    my_yrange(2) = yrange(2)	
	    write ( file_unit, '(a, f6.1, a, f6.1, a)' ) &
	        'set yrange [', my_yrange(1), ':', my_yrange(2), ']'
	endif
!***********************************************************************************
	if (present(style)) then
...
!***********************************************************************************
!***********************************************************************************
	subroutine plot_3(x1,y1,x2,y2,x3,y3,style,pause,color1,color2,color3, &
                           terminal,filename,polar,persist,input,linewidth, &
	                       xrange, yrange, logscale)
!***********************************************************************************
...
!***********************************************************************************
	...
	real(kind=4), optional		:: pause,linewidth
	character(len=*),optional	:: style, color1, color2, color3, terminal, &
                                      filename, polar, persist, input, logscale
    real(kind=8), intent(in), optional    :: xrange(2), yrange(2)
...
!***********************************************************************************
	...
	if (present(logscale)) then
		if (logscale .eq. 'x')  write ( file_unit, '(a)' ) 'set logscale x'
		if (logscale .eq. 'y')  write ( file_unit, '(a)' ) 'set logscale y'
		if (logscale .eq. 'xy') write ( file_unit, '(a)' ) 'set logscale'
	end if
!***********************************************************************************
	if (present(style)) then
		write ( file_unit, '(a,i2,a)' ) 'plot "' // trim (data_file_name) &
...
!***********************************************************************************
	subroutine plot_2(x1,y1,x2,y2,style,pause,color1,color2, &
	                  terminal,filename,polar,persist,input,linewidth, &
	                  xrange, yrange, logscale, errors)
!***********************************************************************************
! this subroutine plots 2 two-dimensional graphs in the same coordinate system
!***********************************************************************************
	implicit none
	real(kind=8), intent(in)	        :: x1(:), y1(:), x2(:), y2(:)
	real(kind=8), intent(in), optional  :: errors(:)
	real(kind=4), optional		        :: pause,linewidth
	character(len=*),optional	        :: style, color1, color2, terminal, filename, polar, persist, input, logscale
    real(kind=8), intent(in), optional  :: xrange(2), yrange(2)
...
!***********************************************************************************
	...
	if (present(errors)) then
	    do i=1,Nmax
		    write (file_unit,'(5E15.7)') x1(min(i,Nx1)), y1(min(i,Nx1)), errors(min(i,Nx1)), &
		                                 x2(min(i,Nx2)), y2(min(i,Nx2))
	    end do

    else
	    do i=1,Nmax
		    write (file_unit,'(5E15.7)') x1(min(i,Nx1)), y1(min(i,Nx1)), 0.0, &
		                                 x2(min(i,Nx2)), y2(min(i,Nx2))
	    end do
	end if
!***********************************************************************************	
...
!***********************************************************************************
	if (present(style)) then
		write ( file_unit, '(a,i2,a)' ) 'plot "' // trim (data_file_name) &
		&//'" using 1:2:3 with errorbars' // ' pointtype ' // &
		& style(1:2) // ' linecolor rgb ' // trim(my_color1) // ' linewidth '// trim(my_linewidth) // ',\' 
		write ( file_unit, '(a,i2,a)' ) '     "'// trim (data_file_name) &
		&//'" using 4:5 with ' // trim(my_line_type2) // ' pointtype ' &
		&// style(4:5) // ' linecolor rgb ' // trim(my_color2) // ' linewidth '// trim(my_linewidth) 
	else 
		write ( file_unit, '(a,i2,a)' ) 'plot "' // trim (data_file_name) &
		& //'" using 1:2 with ' // trim(my_line_type1)  // ' linecolor rgb '&
...
!***********************************************************************************
	...
	real(kind=4), optional		        :: pause,linewidth
	character(len=*),optional           :: style, color1, terminal, filename, &
                                           polar, persist, input, logscale
    real(kind=8), intent(in), optional  :: xrange(2), yrange(2)
